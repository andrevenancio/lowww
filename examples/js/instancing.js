...
class Main {
    ...
    init() {
        const vertex = `#version 300 es
            ...
            void main() {
                ...
                offset.z = mod(offset.z + (u_time * speed), u_distance) - u_distance / 2.0;

                vec2 dir = normalize(offset.xy);
                dir = rotate(dir, PI * 0.5);

                vec3 axis = normalize(a_color);
                float angle = a_color.z + u_time * mix(a_color.x, 1.0, 0.5) * 5.0;
        ...
        this.model.setUniform('u_time', 'float', this.time);
    ...
    update() {
        this.time += 0.01;
        this.model.uniforms.u_time.value = this.time;
